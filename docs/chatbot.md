...
| `public` | **같은 팀의 모든 사용자**가 대화 가능. 단, 문서는 챗봇에 연결된 것만 바라봄(오너 관점에서 "내가 넣은 문서"). |
...
| `PATCH`| `/chatbots/{id}` | 이름/프롬프트/스코프/가시성 등 수정 |
...
1. `chatbot_id`가 있으면 로드 & 권한 체크
 - `private`이면 `chatbot.owner_user_id == user.id` 여야 함
 - `public`이면 `chatbot.team_id == user.team_id` 여야 함
