...
| skeleton | `<Skeleton>` / `<SkeletonList>` (`shared/ui/Skeleton.tsx`) 또는 `stitch-skeleton` 클래스 | `role="status"` + `sr-only` 안내 |
| toast (일상 피드백) | `toast()` (`shared/ui/Toast.tsx`) — 저장/삭제/실패 등 비차단 즉시 피드백 | success/error/info 토큰, 우하단(`bottom-6 right-6`), `role="status"`·자동 소거 |
...
> **레이아웃 표준 (`globals.css`)**: 본문 컨테이너는 두 단계만 쓴다 — `.page-shell`(72rem · 목록/관리/그리드)과 `.page-shell--narrow`(56rem · 읽기/폼/상세). 세로 리듬·가로 패딩·바닥 여유가 고정돼 화면 전환 시 본문 폭이 점프하지 않으며, AppShell 헤더도 같은 폭에 정렬한다.
